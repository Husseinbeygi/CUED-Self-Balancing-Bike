...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}
			...
			\addplot[mark=none] table[x=t,y=phi, col sep=comma] {pidLego.csv};		
		\end{axis}
	\end{tikzpicture}
	...
	\begin{tikzpicture}
		\begin{axis}
			[xlabel=Time (s),
			 ...
			 tick label style={/pgf/number format/fixed}]
			\addplot[mark=none] table[x=t,y=delta, col sep=comma] {pidLego.csv};
		\end{axis}
	\end{tikzpicture}
...
As can be seen from the plots, the PID controller in simulation can recover from an impulse in approximately one second whilst keeping the controller effort relatively small.
...
\begin{figure}[H]
	\begin{tikzpicture}[auto, node distance=2cm,>=latex']
		\node [sum] (sum) {};
		\node [block, right of=sum] (actuator) {$G_a(s)$};
    	\node [block, right of=actuator] (plant) {$G(s)$};
    	\node [output, right of=plant] (output) {};
    	\node [block, below of=plant] (obsv) {Observer};
    	\node [block, left of=obsv, node distance=2.5cm] (lqr) {\textbf{K}};

		\draw [->] (sum) -- node[name=u] {$\bar{u}$}(actuator);
    	\draw [->] (actuator) -- (plant);
    	\draw [->] (plant) -- node [name=y] {$\bar{y}$}(output);
    	\draw [->] (output) |- (obsv);
		\draw [->] (obsv) -- node[name=xhat] {$\hat{\bar{\underline{x}}}$} (lqr);    
    	\draw [->] (lqr) -| node[pos=0.99] {$-$} node [near end] {} (sum);
		\draw [->] (u) -| (obsv);    	
